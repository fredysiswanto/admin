...
export async function fetchTransactionCustomers() {
  ...
  try {
    const data = await sql`
      ...
      FROM customers as c RIGHT JOIN invoices as i ON c.id = i.customer_id  
      GROUP BY c.id
        `;

    const datas = data.rows.map((customer) => ({
      ...customer,
      total_pending: formatCurrency(customer.total_pending),
      total_paid: formatCurrency(customer.total_paid),
    ...
    console.error('Failed to fetch transactions customers:', error);
    throw new Error('Failed to fetch transactions customers.');
  }
}
